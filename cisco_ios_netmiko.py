...
def show_command_fun(ssh_device):
        ...
        try:
                net_connect = ConnectHandler(device_type="cisco_xr",host=ssh_device, username=username, password=password,global_delay_factor=2)
                print(f"***** connected to {ssh_device}")
                for sh_cmd in commands_list:
                        output = net_connect.send_command(sh_cmd,strip_command=False,strip_prompt=False)
                        sh_cmd1 = sh_cmd.replace(" ", "_").strip()
                        file_name = file_path+f"/{formated_time}/{ssh_device1}/{ssh_device1}_{sh_cmd1}.txt"
                ...
                file_name = file_path+f"/{formated_time}/{ssh_device1}/{ssh_device1}_Exception.txt"
                with open(file_name, 'w+') as file_open:
                        file_open = open(file_name, 'w+')
                        file_open.write(f"{ssh_device} Device Not Rechable \n")
username = input("Enter username : ")
password = getpass.getpass("Enter Password : ")
with open("commands.txt") as commands, open("hostname.txt") as hostnames:
